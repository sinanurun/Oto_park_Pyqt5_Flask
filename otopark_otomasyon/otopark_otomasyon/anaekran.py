...
class Anaekran(QMainWindow, Anapencere_Ui):
    ...
    def fhesapla(self):
        ...
        self.zaman_farki = zaman_farki if zaman_farki>0 else 1
        try:
            self.zaman_carpani = session.query(Zamankatsayilari).filter(Zamankatsayilari.zaman_baslangic <= zaman_farki, Zamankatsayilari.zaman_bitis > zaman_farki).first()
        except:
            self.zaman_carpani = session.query(Zamankatsayilari).first()

        brm_ucret = session.query(BirimZaman).first().birimzaman_tutari

        ucret = self.zaman_carpani.zaman_carpani*park.aboneid.abone_katsayisi*park.aracid.arac_katsayisi*brm_ucret*self.zaman_farki
        print(ucret)
        # self.ftablo()
